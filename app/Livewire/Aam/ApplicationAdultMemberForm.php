...
class ApplicationAdultMemberForm extends Component implements HasForms
{
    ...
    public function mount(Request $request): void
    {
        ...
        /*$this->form->fill([
            ...
            'has_given_public_media_consent' => true,
            'scouting_consent' => true,
            'sd_region_id' => null,
            'sd_district_id' => null,
            'sd_group_id' => null,
        ]);*/
                ...
                Wizard::make(
                    [
                        Wizard\Step::make('Contact Info')
                            ...
                            ->schema([
                                TextInput::make('email')
                                    ...
                                    ->unique('sd_core.system_users', 'username')
                                    ->unique('mysql.application_adult_membership_requests', 'email')
                                    ->rules([
                                        Rule::unique('sd_core.system_users', 'username'),
                                    ])
                                    ->validationMessages([
                        ...
                        Wizard\Step::make('Personal Info')
                            ->description(fn (Get $get) => Arr::get(UserConstants::TITLES, $get('title') ?? '', null) . ' ' . $get('first_name') . ' ' . $get('surname'))
                            ->columns(2)
                            ->schema([

                                Select::make('title')
                                    ->options(UserConstants::TITLES)
                                    ->placeholder('Mr/Ms/...')
                                    ->searchable()
                                    ->required(),
                        ...
                        Wizard\Step::make('Emergency & Privacy')
                            ->schema([
                                ...
                                Placeholder::make('Use of photographs and audio/audio-visual recordings')
...
<li>17. I have watched the <a class='font-medium text-blue-600 underline dark:text-blue-500 hover:no-underline' href='https://www.youtube.com/watch?v=5lNyx8JmZK8'>WOSM introductory Safe from Harm video</a>. See more: https://www.scout.org/safefromharm</li></ol>")),
                        ...
                        Wizard\Step::make('Group Details')
                            ->schema([
                                Placeholder::make('Who should we notify for your request?')
                                    ->helperText('Please enter the region/district/group which is most accurate for you. This is so that we can notify the correct people of your request.'),

                                Select::make('region')
                                    ->options(V2Region::active()->pluck('name', 'id')->toArray())
                                    ->searchable()
                                    ->live()
                                ...
                                Select::make('district')
                                    ->options(function (Get $get): array {
                                        return V2Region::find($get('region'))?->v2Districts()->active()->pluck('name', 'id')->toArray() ?? [];
                                    })
                                ...
                                Select::make('group')
                                    ->options(function (Get $get): array {
                                        if ($get('district') !== null) {
                                            return V2District::find($get('district'))?->v2Groups()->active()->pluck('name', 'id')->toArray() ?? [];
                                        }
                                        if ($get('region') !== null) {
                                            return V2Region::find($get('region'))?->v2Groups()->active()->pluck('name', 'id')->toArray() ?? [];
                                        }
                                    ...
                                    ->afterStateUpdated(function (Set $set, Get $get) {
                                        if ($get('district') === null) {
                                            $set('district', V2Group::find($get('group'))?->assoc_to_district);
                                        }
                                    })
                                    ->placeholder('First Select a Region')
                                    ->searchable()
...
//                    ->skippable(true),
    ...
    public function create(): void
    {
        ...
        $aam = V3ApplicationAdultMembershipRequest::create([
            ...
            'sd_region_id' => $state['region'] ?? null,
    ...
    {
        return view('livewire.aam.aam_form');
    }
}
