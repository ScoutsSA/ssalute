...
- A member whose profile has the infoRedacted flag set shows the word "Redacted" in place of email and cell number, exactly as the legacy system does. Redaction only affects the directory: scouters who manage the member's group, district or region still see the details on the member's record. Members control the flag themselves from the action menu on their profile page ("Redact my contact info" and "Allow my contact info to be shared", each behind a confirmation modal), and the profile's Contact tab shows the current directory sharing status. In the directory, a redacted member's row offers "Information redacted" in place of "Contact urgently", which explains the situation and links the viewer to their own profile page. A handful of national office holder roles (National Administrator, CEO, Chief Commissioner, Chief Scout) never show a cell number, again matching legacy.
- Tables show name and role by default. For adult leaders the email, cell number, WhatsApp and area columns exist but are toggled off, and each row carries an actions column with two entries. "Contact" (envelope icon) messages the member through the system: a modal with a prefilled subject and message, sent from the system with the reply-to set to the sender. The sender is copied in only when the member has not redacted their details, because a copy would carry the member's address. "Contact urgently", in the row's action group, only exists for members who have not redacted and opens a modal with their email as a mailto link, their cell number as a tel link, and a WhatsApp chat link for South African numbers.
- Every message sent through the system is recorded in the system_contact_messages table and listed in the BackOffice Admin Reports cluster as System Contact Messages. Sending is rate limited per sender.
