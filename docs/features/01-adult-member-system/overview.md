...
- Adult leaders also get a WhatsApp icon next to each cell number that opens a chat with that number (South African numbers only), and a "Contact via the system" action on every listed member who has an email address on file. The action opens a modal with a prefilled subject and message, sends the email from the system with the sender copied in and the reply-to set to the sender, and never reveals a redacted member's address. Every message sent is recorded in the system_contact_messages table and listed in the BackOffice Admin Reports cluster as System Contact Messages. Sending is rate limited per sender.
